...
def audio_to_text_with_retry() -> None:
  '''
  ...
  for attempt in range(MAX_RETRIES):
    try:
      ...
      if attempt < MAX_RETRIES - 1:
        st.warning(f"Rate limit error occurred. Retrying in 5 seconds (attempt {attempt + 1}/{MAX_RETRIES})")
        time.sleep(5)
      else:
  ...
  prompt = """You are a teacher helping teach students with learning disabilities such as Dyslexia and ADHD. 
              The answer provided should always include these 5 sections listed below and should also contain any of the OPTIONAL SECTIONS listed below if applicable: 
                    ...
                    5) Quiz Yourself!: a short quiz with questions on the key takeaways from the text along with the answers. 
                        - multiple choice questions. display all the options
                        - Hide answers by default. Include a button called show answers to display the correct answers after every question.
...
def display_sidebar(text: str) -> None:
    ...
    pattern = r'##\s+(.*?)(?=:)'  # Updated regular expression to exclude colon
...
def app():
  ...
  image = Image.open("assets/images/bg.jpg")
  st.image(image)
  st.title("MINDMATE 📑")
  ...
  with st.sidebar:   
    ...
    image = Image.open("assets/images/mm.jpeg")
    st.image(image)
    
    # Add the header to the sidebar
    st.header("Intelligent Summarization Assistant")
    st.write("_For Neuro Diverse Learners._")
